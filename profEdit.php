...
if(!empty($_POST)){
    ...
    //ポストされた値を変数に入れる
    $image = (!empty($_FILES['pic']['name'])) ? uploadImg($_FILES['pic'], 'pic') : '';
    debug('画像の詳細データ' . print_r($image, true));
    $name = $_POST['username'];
    $age = $_POST['age'];
    $sex = $_POST['sex'];
    ...
    if(empty($err_msg)){
        ...
        try{
            ...
            $data = array(':u_name'=>$name, ':email'=>$email, ':pref'=>$prefecture, ':age'=>$age, 'sex'=>$sex, ':pic' => $image, ':u_id'=>$dbFormData['id']);
    ...
    <main>
        <div class="site-width prof-edit">
            ...
            <div class="prof-img areaDrop">
                <img src=""  class="prof-prev-img prevImg" alt="">
