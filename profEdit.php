...
if(!empty($_POST)){
    ...
    if(empty($err_msg)){
        ...
        try{
            $dbh = dbConnect();
            $sql = 'UPDATE users SET username = :u_name, email = :email, prefecture_id = :pref, age = :age, sex = :sex WHERE id = :u_id';
            $data = array(':u_name'=>$name, ':email'=>$email, ':pref'=>$prefecture, ':age'=>$age, 'sex'=>$sex, ':u_id'=>$dbFormData['id']);
